...
(function (mtd) {
  ...
  var partDetail = mtd.partDetail;

  // shared component variables
  var $partDetail = $('.part-detail'),
      $partTabs = $('.parts-tabs'),
      $tabs = $partTabs.find('.tabs'),
      $tabContent = $partTabs.find('.tab-content'),
      colorsClass = 'colors a',
      selectedStyleClass = 'selected-style',
      $colors = $partDetail.find('.' + colorsClass),
      $colorSelection = $partDetail.find('#color-select'),
      $colorSelectionText = $partDetail.find('.' + selectedStyleClass),
      activeClass = 'active',
  ...
  partDetail.loader = {
    init: function() {
      partDetail.tabs.init();
      partDetail.colors.init();
    }
  },
  partDetail.tabs = {
    init: function() {
    ...
    click: function() {
      $tabs.on('click', 'a', function(e) {
        ...
        e.preventDefault();
        partDetail.tabs.setActiveTab($(this), hash);
    ...
    handleHashLink: function() {
      ...
      if (hash) {
        ...
        partDetail.tabs.setActiveTab($link, hash);
      }  
    },
  ...
  partDetail.colors = {
    dom : {
      COLOR_INPUT_CONTAINER : "[data-color-input]",
      COLOR_INPUT_DATA_ATTRIBUTE : "color-input",
      COLOR_INPUT : "[data-color]",
      COLOR_VALUE_DATA_ATTRIBUTE : "color",
      SELECTED_INDICATOR_CLASS : ".selected-style",
      ACTIVE_CLASS : "active"
    },
    init : function() {
      this.select();
    ...
    select : function(e) {
      $partDetail.on('click', '.' + colorsClass, function(e) {       
        e.preventDefault();
        var $color = $(this);

        if(! $color.hasClass(activeClass)) {
          var colorChosen = $color.data('color');

          $colors.removeClass(activeClass);
          $color.addClass(activeClass);
          $colorSelectionText.text(colorChosen);
          $colorSelection.val(colorChosen);
        }
      });
    }
  }
